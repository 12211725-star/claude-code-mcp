 ...
 */
function isAllowedCommand(cmd, allowedCommands) {
    return allowedCommands.some((allowed) => {
        // Exact match
        if (allowed === cmd)
            return true;
        // Path match (e.g., "git" matches "/usr/bin/git")
        if (allowed === path.basename(cmd))
            return true;
        return false;
    });
}
 ...
 */
function hasPathTraversal(command, workspaceRoot) {
    ...
    if (absPathMatch) {
        for (const p of absPathMatch) {
            if (p === workspaceRoot || p.startsWith(workspaceRoot))
                continue;
            // Common safe absolute paths
            if (["/usr/bin", "/usr/local/bin", "/bin", "/dev/null", "/tmp"].some((safe) => p === safe || p.startsWith(safe)))
                continue;
 ...
 */
export function sanitizeOutput(output, maxBytes) {
    return output
 ...
 */
export function maskSecrets(command) {
    return command
