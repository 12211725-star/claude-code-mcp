...
- 保持原有缩进，不要改变缩进风格`,
    ...
    async handler(args, ctx) {
        ...
        if (occurrences > expected_replacements) {
            throw new Error(`old_string 在文件中出现了 ${occurrences} 次，预期 ${expected_replacements} 次。\n` +
                `请提供更多上下文使匹配唯一。`);
        }
        // Execute replacement (replace first occurrence)
        const newContent = content.replace(old_string, new_string);
        // Generate diff
        const diff = createTwoFilesPatch(file_path, file_path, content, newContent, "原始文件", "编辑后");
        // Write file
...
function findSimilarStrings(content, search) {
...
//# sourceMappingURL=smartEdit.js.map