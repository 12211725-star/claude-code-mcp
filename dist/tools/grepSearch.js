...
- 支持 -A/-B/-C 上下文行数`,
    ...
    async handler(args, ctx) {
        const { pattern, path: searchPath, glob: globPattern, contextBefore = 0, contextAfter = 0, contextAround = 0, ignoreCase = false, maxResults = ctx.config.tools.grepSearch.maxResults, } = args;
        ...
        if (args.include) {
            const exts = args.include.split(",").map((e) => e.trim());
            filteredFiles = allFiles.filter((f) => exts.some((ext) => f.endsWith(ext)));
        }
