...
export interface AppConfig {
  ...
  tools: {
    smartEdit: { enabled: boolean; maxFileSize: number };
    globSearch: { enabled: boolean; maxResults: number };
    grepSearch: { enabled: boolean; maxResults: number };
    bashExec: { enabled: boolean };
    taskAgent: { enabled: boolean; maxConcurrent: number };
  };
}
